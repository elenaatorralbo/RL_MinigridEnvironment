...
def run_lava_curriculum_max1():
    ...
    for i, config in enumerate(stages_config):
        ...
        print(f"\n--------------------------------------------------")
        print(f"🌋 PREPARANDO {stage_name}")
        # print(f"   Habitaciones: {n_rooms} | Lava: {lava_prob * 100}% | Pasos: {steps}") # Redundante con la barra
        ...
        if model is None:
            ...
            # print(f"🧠 Transfiriendo agente...") # Limpieza visual
            model.set_env(env)
        ...
        checkpoint_callback = CheckpointCallback(
            ...
            verbose=0  # <--- Verbose 0 para no romper la barra visual
        )
        ...
        # Usamos tqdm.write para imprimir debajo de la barra limpiamente
        tqdm.write(f"✅ Completado. Guardado en {final_save_name}.zip")
        env.close()
