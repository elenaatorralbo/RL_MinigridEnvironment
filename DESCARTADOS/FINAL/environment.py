...
class SurvivalCorridorEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        for i in range(self.num_rooms):
            ...
            if i >= 10:
                lava_prob = min(1.0, (i - 9) * 0.1)
                rx, ry = x_start + (self.room_size // 2), center_y
                if random.random() < lava_prob: self.grid.set(rx, ry, Lava())

            if i < self.num_rooms - 1:
                self.grid.vert_wall(x_end, 0)
                if random.random() < min(1.0, (i + 1) * 0.1):
                    color = available_colors[i % len(available_colors)]
                    self.grid.set(x_end, center_y, Door(color, is_locked=True))
                    try: self.place_obj(AutoKey(color), top=(x_start, 0), size=(self.room_size, self.room_size))
                    except: pass
                else: self.grid.set(x_end, center_y, None)
        ...
        # --- CAMBIO IMPORTANTE ---
        # Inicializamos el récord de distancia con la posición inicial
        self.max_x_reached = self.agent_pos[0]
        # Eliminamos self.last_x, ya no sirve
