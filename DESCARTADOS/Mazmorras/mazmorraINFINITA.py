...
# Importamos tu clase personalizada (asegúrate de que esté en el mismo archivo o impórtala)
# class InfiniteRoomsEnv(MiniGridEnv): ... (el código anterior)

from stable_baselines3 import PPO
from minigrid.wrappers import FlatObsWrapper

def train_enhanced():
    env = InfiniteRoomsEnv(num_rooms=50) # Empezamos con un reto largo
    env = FlatObsWrapper(env)

    # El hiperparámetro 'ent_coef' ayuda a que el agente explore más 
    # y no se quede estancado por miedo a la lava al principio.
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    print("Entrenando con recompensas de navegación...")
    model.learn(total_timesteps=2000000)
    model.save("ppo_infinite_navigator")
